...
class DepthwiseUnet(UnetBase):
    """
    深度可分离卷积模块结合Unet，在保证准确率的前提下，减少参数量
    """

    def __init__(self, in_channel, out_channels):
        super(DepthwiseUnet, self).__init__(in_channel, out_channels)
    ...
    def contract_block(in_channels, out_channels):
        ...
        """
        block = torch.nn.Sequential(
            DepthwiseUnet.base_block(in_channels, out_channels),
    ...
    def expansive_block(in_channels, mid_channels, out_channels):
        ...
        """
        block = torch.nn.Sequential(
            DepthwiseUnet.base_block(in_channels, mid_channels),
